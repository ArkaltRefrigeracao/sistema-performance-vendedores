...
def get_pontuacoes_mensais():
    ...
    query_overall = PontuacaoVendedor.query.options(joinedload(PontuacaoVendedor.vendedor)).filter(
    ...
    overall_labels = [p.vendedor.nome for p in overall_performance_data]
    ...
    # --- DADOS PARA O HISTÓRICO DE LANÇAMENTOS (LÓGICA MODIFICADA) ---
    historico_lancamentos = []
    
    # Query base que filtra por mês e ano
    query_base = RegistroDesempenho.query.options(
        joinedload(RegistroDesempenho.quesito_rel),
        joinedload(RegistroDesempenho.vendedor_do_registro)
    ).filter(
        func.strftime('%Y', RegistroDesempenho.data_registro) == str(ano),
        func.strftime('%m', RegistroDesempenho.data_registro) == f"{mes:02d}"
    )

    # Se um vendedor foi selecionado no filtro, adiciona à query
    if vendedor_id_str and vendedor_id_str != 'all':
        try:
            vendedor_id_int = int(vendedor_id_str)
            query_base = query_base.filter(RegistroDesempenho.vendedor_id == vendedor_id_int)
        except ValueError:
            return jsonify({'error': 'ID do vendedor inválido para o histórico.'}), 400

    # Ordena por data mais recente e busca todos os registros
    registros = query_base.order_by(RegistroDesempenho.data_registro.desc()).all()
    ...
    for registro in registros:
        ...
        historico_lancamentos.append({
            ...
            'vendedor': registro.vendedor_do_registro.nome,
            'quesito': registro.quesito_rel.nome,
            'pontuacao_padrao': registro.quesito_rel.pontuacao_padrao,
    ...
    return jsonify({
        ...
        'launch_history': historico_lancamentos
    })